...
async function cargarFase(){
  try{
    ...
    var s = Number(CLIENTE.semana)||0;
    if(!s){ __FASE=null; return; }
    var r = await sb.from("saneas_comentarios_fase").select("*")
      .lte("semana", s).gte("semana_hasta", s).eq("publicado", true).limit(1);
...
function pintarFase(){
  ...
  var sub = "Semanas "+__FASE.semana+"-"+__FASE.semana_hasta+(__FASE.fase?" · "+__FASE.fase:"");
